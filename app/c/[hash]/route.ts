...
export async function GET(
  ...
  try {
    ...
    const eventType =
      isClick && trackingLink.track_clicks
        ? "click"
        : trackingLink.track_impressions
          ? "impression"
          : null;
      ...
      if (insertResult.data) {
        ...
        if (eventType === "click") {
          ...
          fetch(`${baseUrl}/api/track/enrich`, {
            ...
            .catch((err) =>
              console.error("[CLICK] ❌ Enrichment error (non-critical):", err),
            );
        }
    ...
    if (
      ...
      trackingLink.destination_url.trim() === ""
    ) {
    ...
    const creatorId = (trackingLink as any).collaborations?.[0]?.applications
      ?.creator_id;
    if (creatorId) {
