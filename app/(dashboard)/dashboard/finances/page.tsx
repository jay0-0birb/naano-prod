...
export default async function FinancesPage({ searchParams }: PageProps) {
  ...
  if (isCreator) {
    ...
    const { data: creatorProfile } = await supabase
      ...
      .select(
        "id, subscription_tier, stripe_account_id, stripe_onboarding_completed, is_pro, pro_status_source, pro_expiration_date, stripe_subscription_id_pro, legal_status, siret_number",
      )
    ...
    const { data: saasCompany } = await supabase
      .from("saas_companies")
      .select(
        "id, company_name, subscription_tier, subscription_status, stripe_subscription_id, card_on_file, card_last4, card_brand, wallet_credits, monthly_credit_subscription, credit_renewal_date, stripe_subscription_id_credits",
      )
      .eq("profile_id", user.id)
      .single();
