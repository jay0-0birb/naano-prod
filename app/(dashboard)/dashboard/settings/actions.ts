...
async function processLogoForAvatar(buffer: ArrayBuffer, contentType: string): Promise<Buffer> {
  ...
  try {
    ...
    if (aspect > 1.3 || aspect < 0.77) {
      ...
      pipeline = sharp(trimmed).extract({ left, top, width: size, height: size })
    }
...
export async function fetchLogoFromWebsite(websiteUrl: string) {
  ...
  try {
    ...
    if (!imgRes.ok) return { error: 'Impossible de récupérer le favicon. Vérifiez l\'URL.' }
    const contentType = imgRes.headers.get('content-type') || ''
    if (!contentType.startsWith('image/')) return { error: 'Aucun favicon trouvé pour ce site.' }

    const arrayBuffer = await imgRes.arrayBuffer()
    if (arrayBuffer.byteLength > AVATAR_MAX_SIZE) return { error: 'L\'image est trop volumineuse (max 2 Mo)' }

    const trimmedBuffer = await processLogoForAvatar(arrayBuffer, contentType)
    if (trimmedBuffer.length > AVATAR_MAX_SIZE) return { error: 'L\'image est trop volumineuse (max 2 Mo)' }
    ...
    if (uploadError) return { error: 'Erreur lors de l\'enregistrement du logo' }
    ...
    if (e instanceof Error) {
      if (e.name === 'AbortError') return { error: 'Délai dépassé. Vérifiez l\'URL.' }
