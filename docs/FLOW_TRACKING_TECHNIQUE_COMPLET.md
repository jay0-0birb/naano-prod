...
L'export CSV généré côté client reprend ces champs dans des colonnes : date, heure, créateur, IP masquée, pays, ville, appareil, OS, navigateur, type de réseau, temps sur site, référent, session ID, nom entreprise, domaine, industrie, taille, localisation, score de confiance, confiance effective (avec décroissance temporelle pour les inférés), état d'attribution, raisons, ASN, ambiguïté, dates d'enrichissement et confirmation, scores d'intention session et entreprise, tendance, visite répétée, nombre de visites, heures de travail, poids de récence, jours depuis session, flags pages vues, agrégats entreprise. L'IP est masquée (par exemple dernier octet remplacé) pour la conformité RGPD.
